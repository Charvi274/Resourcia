...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate(res.data.user.role === 'admin' ? '/dashboard' : '/assets');
    } catch (err) {
      ...
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '28px' }}>
            Use <span style={{ color: 'var(--pink-soft)' }}>admin@admin.com</span> for admin access
          </p>
        ...
        <p style={{ marginTop: '20px', textAlign: 'center', fontSize: '13px', color: 'var(--text-secondary)' }}>
          No account?{' '}
          <Link to="/register" style={{ color: 'var(--pink-primary)', textDecoration: 'none', fontWeight: 500 }}>Create one</Link>
        </p>
      </div>
    </div>
  );
